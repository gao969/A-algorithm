...
if __name__ == '__main__':
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for cell in cell_idx:
            ...
            w, loss, epoch_loss, local_delta_c, local_delta, control_local_w, ww, grad_np_dgc = local_model.update_weights_topk(model=copy.deepcopy(global_model),
                                                             global_round=epoch, control_local=c_local[cell], control_global = control_global, w_loc = w_locals[cell])
            if epoch != 0:
        ...
        for w in f_global_weights:
            global_weights[w] += f_global_weights[w]
    ...
    print('Scaffold File: {:} Type: {:} MSE: {:.4f} MAE: {:.4f}, NRMSE: {:.4f}'.format(args.file, args.type, mse, mae,
                                                                                     nrmse))
    plt.figure()
    plt.plot(range(len(loss_hist)), loss_hist, label='Scaffold-Topk')
    plt.ylabel('train_loss')
    plt.xlabel('epochs')
    plt.legend(loc='best')
    plt.savefig(
        './save/fed_{}.png'.format('train_loss'))
