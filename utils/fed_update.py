...
class LocalUpdate(object):
    ...
    def update_weights_sg(self, model, global_round, control_local, control_global):
        ...
        for w in net_weights:
            new_control_local_w[w] = new_control_local_w[w] - control_global_w[w] + (
                        global_weights[w] - net_weights[w]) / (count * self.args.lr)
            control_delta[w] = new_control_local_w[w] - control_local_w[w]
            local_delta[w] -= (global_weights[w]/self.train_loader.batch_size)
    ...
    def update_weights_topkp(self, model, global_round, control_local, control_global, w_loc, c_local_epoch):
        global_weights = copy.deepcopy(model.state_dict())
        control_global_w = copy.deepcopy(control_global.state_dict())
        control_local_w = control_local.state_dict()
        count = 0
        ...
        if global_round != 99:
            if c_local_epoch % 5 == 0:
                for w in local_weight:
                    d_weight[w] = d_weight[w] - d_weight[w]
                    d_weight[w] = w_weight[w]
                    w_weight[w] = w_weight[w] - w_weight[w]
            else:
                for w in local_weight:
                    d_weight[w] = d_weight[w] - d_weight[w]
                    sample_ratio = 0.01
        ...
        return model.state_dict(), sum(epoch_loss)/len(epoch_loss), epoch_loss, control_delta, d_weight, new_control_local_w,  w_weight, grad_np_dgc, c_local_epoch+1

    def update_weights_topkp_p10(self, model, global_round, control_local, control_global, w_loc, c_local_epoch):
        global_weights = copy.deepcopy(model.state_dict())
        control_global_w = copy.deepcopy(control_global.state_dict())
        control_local_w = control_local.state_dict()
        count = 0
    ...
    def update_weights_topkp_p15(self, model, global_round, control_local, control_global, w_loc, c_local_epoch):
        global_weights = copy.deepcopy(model.state_dict())
        control_global_w = copy.deepcopy(control_global.state_dict())
        control_local_w = control_local.state_dict()
        count = 0
    ...
    def update_weights_topkp_p0(self, model, global_round, control_local, control_global, w_loc, c_local_epoch):
        global_weights = copy.deepcopy(model.state_dict())
        control_global_w = copy.deepcopy(control_global.state_dict())
        control_local_w = control_local.state_dict()
        count = 0
