...
from utils.update_MLP import LocalUpdate, test_inference
...
if __name__ == '__main__':
    ...
    global_model = MLP(args).to(device)
    control_global = MLP(args).to(device)
    global_model.train()
    # print(global_model)
    topk_global_model = MLP(args).to(device)
    topk_control_global = MLP(args).to(device)
    ...
    GCAU_global_model = MLP(args).to(device)
    GCAU_control_global = MLP(args).to(device)
    ...
    GCAU_control_weights = GCAU_control_global.state_dict()
    GCAU_global_weights = GCAU_global_model.state_dict()
    GCAU_c_glob = copy.deepcopy(GCAU_control_weights)
    ...
    for idx in selected_cells:
        c_local[idx] = MLP(args).to(device)
        c_local[idx].load_state_dict(control_weights)
        c_local_epoch[idx] = 0
        topk_c_local[idx] = MLP(args).to(device)
        ...
        GCAU_c_local[idx] = MLP(args).to(device)
    ...
    GCAU_delta_c = copy.deepcopy(GCAU_global_model.state_dict())
    ...
    for w in global_weights:
        ...
        GCAU_weight[w] = GCAU_global_weights[w] - GCAU_global_weights[w]
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for cell in cell_idx:
            ...
            w, loss, epoch_loss, local_delta_c, local_delta, control_local_w, ww, grad_np_dgc, c_local_epoch[cell] = local_model.update_weights_A(model=copy.deepcopy(global_model),
            ...
            topk_w, topk_loss, topk_epoch_loss, topk_local_delta_c, topk_local_delta, topk_control_local_w, topk_ww, topk_grad_np_dgc = local_model.update_weights_topkG(
            ...
            top_w, top_loss, top_epoch_loss, top_local_delta_c, top_local_delta, top_control_local_w, top_ww = local_model.update_weights_topk(
            ...
            GCAU_w, GCAU_loss, GCAU_epoch_loss, GCAU_local_delta_c, GCAU_local_delta, GCAU_control_local_w, GCAU_ww,  GCAU_c_local_epoch[
                cell] = local_model.update_weights_GCAU(model=copy.deepcopy(GCAU_global_model),
                                                             global_round=epoch, control_local=GCAU_c_local[cell],
                                                             control_global=GCAU_control_global, w_loc=GCAU_w_locals[cell],
        ...
        for w in weight:
            ...
            GCAU_global_weights[w] += average_weights(GCAU_local_weights)[w]
        ...
        GCAU_control_global_w = GCAU_control_global.state_dict()
        for i in control_global_w:
            ...
            GCAU_control_global_w[i] += (1 / m) * GCAU_delta_c[i]
    ...
    print('GCAU File: {:} Type: {:} MSE: {:.4f} MAE: {:.4f}, NRMSE: {:.4f}'.format(args.file, args.type, GCAU_mse,
                                                                                     GCAU_mae,
                                                                                     GCAU_nrmse))
    ...
    plt.plot(range(len(GCAU_loss_hist)), GCAU_loss_hist, label='GCAU')
    ...
    plt.legend(loc='best')
    plt.savefig(
        './save/experimental_comparison_MLP_{}.png'.format('train_loss'))