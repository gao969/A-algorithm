...
if __name__ == '__main__':
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for cell in cell_idx:
            ...
            w, loss, epoch_loss, local_delta_c, local_delta, control_local_w, ww, grad_np_dgc, c_local_epoch[cell] = local_model.update_weights_topkp(model=copy.deepcopy(global_model),
                                                             global_round=epoch, control_local=c_local[cell], control_global = control_global, w_loc=w_locals[cell], c_local_epoch=c_local_epoch[cell])
            if epoch != 0:
    ...
    print('Scaffold_TopK.p_p File: {:} Type: {:} MSE: {:.4f} MAE: {:.4f}, NRMSE: {:.4f}'.format(args.file, args.type, mse, mae,
                                                                                     nrmse))
    plt.figure()
    plt.plot(range(len(loss_hist)), loss_hist, label='Scaffold-TopK.p_p')
    plt.ylabel('train_loss')
    plt.xlabel('epochs')
    plt.legend(loc='best')
    plt.savefig(
        './save/scaffold_TopK.p_p_{}.png'.format('train_loss'))
