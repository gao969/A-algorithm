...
from utils.models import LSTM, MLP
...
if __name__ == '__main__':
    ...
    top_control_weights = initialize_parameters_zeros(top_control_global)
    ...
    print(
        'Scaffold_TopK File: {:} Type: {:} MSE: {:.4f} MAE: {:.4f}, NRMSE: {:.4f}'.format(args.file, args.type, top_mse, top_mae,
                                                                                     top_nrmse))

    plt.figure()
    plt.plot(range(len(fed_loss_hist)), fed_loss_hist, label='FedAvg')
    plt.plot(range(len(top_loss_hist)), top_loss_hist, label='Scaffold_TopK')
    plt.ylabel('train_loss')
    plt.xlabel('epochs')
    plt.legend(loc='best')
    plt.savefig(
        './save/t_scaffold_TopK_{}.png'.format('train_loss'))